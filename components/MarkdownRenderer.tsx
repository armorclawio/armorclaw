    ...
    return tokens.map((token, i) => {
        if (token.startsWith('`') && token.endsWith('`') && token.length > 2) {
            return (
                <code
                    key={i}
                    className="bg-black/20 dark:bg-white/10 text-accent px-1.5 py-0.5 rounded text-[0.85em] font-mono border border-black/10 dark:border-white/10"
                >
                    {token.slice(1, -1)}
                </code>
            );
    ...
    while (i < lines.length) {
        ...
        if (fenceMatch) {
            ...
            nodes.push(
                <pre
                    key={nodes.length}
                    className="bg-black/20 dark:bg-black/40 text-[0.82rem] md:text-sm p-4 rounded-xl overflow-x-auto my-3 border border-white/10 font-mono leading-relaxed"
                >
                    {lang && (
